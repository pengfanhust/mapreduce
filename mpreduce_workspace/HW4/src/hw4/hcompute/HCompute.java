...
public class HCompute {
	...
	public static class HComputeMapper extends TableMapper<RecordKey, Record> {
		...
		public void map(ImmutableBytesWritable row, Result value,
			...
			String year = new String(line.get(Constants.COL_YEAR));
			// year is not be 2008
			if (!year.equals(Constants.YEAR))
				return;

			String cancelled = new String(line.get(Constants.COL_CANCELLED));
			String diverted = new String(line.get(Constants.COL_DIVERTED));
			// has be cancelled or diverted
			if (!(cancelled.equals(Constants.N_CANCELLED) && diverted
	...
	public static void main(String[] args) throws IOException,
		...
		Job job = new Job(config, "flight delay hcompute");
